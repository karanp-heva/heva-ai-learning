...
const QUESTIONS = [
  ...
  // ═══ DEEP SCENARIO: Customer Support Resolution Agent (15 questions) ═══
  { id:46,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A telecom support agent handles billing disputes. A customer says 'My bill is $200 higher than normal.' The agent retrieves the account and finds a legitimate one-time installation fee. The agent should:",options:["Apologize and issue a courtesy credit to avoid conflict","Explain the charge clearly, reference the service agreement, and offer to set up a payment plan if needed","Escalate to a billing specialist immediately since the customer is unhappy","Ask the customer to call back during business hours for billing questions"],correct:1,explanation:"The agent should resolve within its capability by explaining the legitimate charge with context and offering practical next steps. Unnecessary escalation or unwarranted credits are not appropriate when the charge is valid."},
  { id:47,domain:"D1",scenario:"Customer Support Resolution Agent",question:"During a support interaction, the agent calls the 'check_order_status' tool and receives: { status: 'error', message: 'Service temporarily unavailable' }. The agent should:",options:["Tell the customer: 'I'm unable to help right now, please try again later'","Retry the tool call once with backoff, and if it fails again, inform the customer with specifics: 'Our order system is temporarily down — I've noted your request and you'll receive an update within 2 hours via email'","Silently switch to a different topic and hope the customer forgets","Fabricate a plausible order status based on the customer's description"],correct:1,explanation:"Retry with backoff is correct, followed by transparent communication with a concrete follow-up plan. Generic deflection, topic-switching, and fabrication are all anti-patterns."},
  { id:48,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A bank's support agent has tools: check_balance, transfer_funds, freeze_card, and close_account. A customer says: 'Someone stole my card, freeze it NOW and transfer my money to my savings!' The correct execution order is:",options:["Transfer funds first (protect the money), then freeze the card","Freeze the card immediately via the freeze_card tool, then execute the transfer_funds tool","Ask the customer to verify their identity first, then freeze, then transfer","Escalate immediately — fraud cases always require human agents"],correct:1,explanation:"Freeze first to stop further unauthorized transactions (the immediate safety action), then handle the transfer. Identity verification may already be complete. Not all fraud cases need escalation if the agent has the right tools."},
  { id:49,domain:"D1",scenario:"Customer Support Resolution Agent",question:"An e-commerce support agent is resolving a refund. The agent calls 'process_refund' with amount: $450. A programmatic hook intercepts because the company policy requires manager approval for refunds > $200. What should happen next?",options:["The hook blocks the tool call and returns a structured message: { blocked: true, reason: 'REFUND_EXCEEDS_LIMIT', limit: 200, required_action: 'manager_approval' } — the agent informs the customer that approval is being requested","The hook silently reduces the refund to $200 and processes it","The agent should have checked the policy before calling the tool — this is a prompt failure","The hook logs a warning but allows the refund to proceed"],correct:0,explanation:"The hook correctly blocks the tool call and returns structured context. The agent then communicates transparently. Silently modifying amounts or allowing violations defeats the purpose of programmatic enforcement."},
  { id:50,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A support agent handles returns for an electronics retailer. A customer wants to return a laptop purchased 45 days ago. The return policy is 30 days. The customer says: 'I've been a loyal customer for 10 years.' The agent should:",options:["Make an exception because customer loyalty matters more than policy","Deny the return firmly and end the conversation","Explain the 30-day policy clearly, acknowledge the customer's loyalty, and offer alternatives within the agent's authority (e.g., store credit, extended warranty claim, escalation to a manager for an exception)","Escalate to a manager without attempting any resolution"],correct:2,explanation:"The agent should be transparent about policy while showing empathy and offering actionable alternatives. Blind exceptions, rigid denials, and premature escalation are all suboptimal."},
  { id:51,domain:"D1",scenario:"Customer Support Resolution Agent",question:"The support agent's system prompt says: 'Always greet the customer warmly and use their first name.' During a conversation, the customer says: 'Stop using my name, it feels fake.' The agent should:",options:["Continue using the name because the system prompt requires it","Stop using the name immediately — customer preferences expressed during the conversation should override style guidelines in the prompt","Escalate because the customer is being difficult","Acknowledge the feedback but explain that it's company policy to use names"],correct:1,explanation:"Runtime customer preferences for interaction style should override soft guidelines in prompts. The system prompt instruction about names is a best-effort guideline, not a hard rule — adapting to the customer's stated preference is better service."},
  { id:52,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A healthcare support chatbot helps patients schedule appointments. A patient says: 'I've been having chest pains for the last hour.' The agent has tools: schedule_appointment, check_availability, cancel_appointment. The agent should:",options:["Schedule the earliest available appointment","Immediately respond with: 'Chest pain can be a medical emergency. Please call 911 or go to your nearest emergency room immediately. Do not wait for a scheduled appointment.' — then offer to help with follow-up scheduling after they've received care","Use sentiment analysis to determine if the patient seems truly distressed","Check appointment availability for today and offer the earliest slot"],correct:1,explanation:"Medical emergency detection must trigger an immediate safety response — this is a hard rule that overrides the agent's normal scheduling workflow. The agent should not attempt to handle emergencies through its standard tools."},
  { id:53,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A support agent is handling 3 concurrent conversations. In conversation A, the customer mentions their account number. In conversation B, a different customer asks about their account. The agent architecture should:",options:["Share context between conversations for efficiency since the agent handles both","Each conversation must have completely isolated context — no information should leak between sessions","Store all account numbers in a shared cache for faster lookups","Let the agent decide which context is relevant to share"],correct:1,explanation:"Context isolation between conversations is critical for security and privacy. Account information from one session must never leak into another. This is a hard architectural requirement, not a preference."},
  { id:54,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A SaaS support agent resolves a customer's login issue by resetting their password. The customer then asks: 'While I have you, can you also update my billing address and cancel my premium subscription?' The agent should:",options:["Handle all requests in sequence since they're all within the agent's capability","Handle the billing address update, but before processing the subscription cancellation, verify this is within the agent's authorized actions and check if cancellation requires additional confirmation or has cooling-off period rules","Decline — the agent should only handle one issue per conversation","Escalate the cancellation to a retention specialist automatically"],correct:1,explanation:"The agent should handle straightforward requests but apply extra caution to high-impact actions like cancellations. Checking whether cancellation has specific business rules (confirmation requirements, cooling-off periods) is the responsible approach."},
  { id:55,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A support agent is designed to handle English-language support. A customer writes their message entirely in Spanish. The agent should:",options:["Attempt to respond in Spanish using its language capabilities","Respond in English and ask the customer to use English","Detect the language mismatch, respond with a brief message in both Spanish and English explaining that the agent is optimized for English support, and offer to transfer to a Spanish-speaking agent or channel if available","Silently ignore the message and wait for an English message"],correct:2,explanation:"The agent should be transparent about its capabilities, provide helpful guidance in both languages, and offer a concrete alternative. Silently attempting Spanish or ignoring the message are both poor experiences."},
  { id:56,domain:"D1",scenario:"Customer Support Resolution Agent",question:"An insurance support agent is processing a claim. Midway through, the customer provides information that contradicts what they said earlier (they first said the incident was on Monday, now they say Tuesday). The agent should:",options:["Use the most recent information and ignore the contradiction","Flag the discrepancy explicitly: 'I noticed you initially mentioned Monday but now said Tuesday. Could you confirm the correct date? This is important for processing your claim accurately.' — then continue with the confirmed information","Report the customer for potential fraud","Silently note both dates in the claim and let the claims processor sort it out"],correct:1,explanation:"Transparently flagging contradictions is the responsible approach. It could be a simple mistake, and clarifying ensures accurate claim processing. Silent acceptance or fraud accusations are both inappropriate responses."},
  { id:57,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A support agent has been helping a customer for 15 messages. The conversation is going in circles — the customer keeps restating the same problem and the agent keeps offering the same solution. The agent should:",options:["Continue repeating the solution more emphatically","Recognize the pattern, summarize what's been attempted so far, acknowledge the customer's frustration, and suggest a different approach or escalate: 'I can see we haven't been able to resolve this with the steps we've tried. Let me connect you with a specialist who can look at this differently.'","End the conversation and close the ticket since a solution was provided","Send the knowledge base article link and close the chat"],correct:1,explanation:"Recognizing circular conversations and changing strategy (different approach or escalation) is the right pattern. Repeating the same solution, prematurely closing, or dumping knowledge base links don't resolve the customer's actual problem."},
  { id:58,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A support agent for a food delivery app receives: 'My order arrived but the bag was open and food was spilled everywhere.' The agent has tools: issue_refund, send_reorder, apply_credit, get_order_details. Best approach:",options:["Immediately issue a full refund without checking the order","Call get_order_details to retrieve the order specifics, then offer the customer a choice: full refund OR free reorder of the same items, plus apply a courtesy credit for the inconvenience. Document the issue with the delivery for quality tracking.","Ask the customer to send photos as proof before doing anything","Tell the customer to contact the restaurant directly"],correct:1,explanation:"Retrieving order details first gives the agent context. Offering choices empowers the customer, and documenting the delivery issue enables systemic improvement. Demanding proof for a food quality issue creates unnecessary friction."},
  { id:59,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A support agent needs to look up a customer's account but the customer can only remember their email address approximately: 'It's something like john.smith@gmail or maybe johnsmith@gmail.com.' The agent should:",options:["Search for both variations, present any matches found, and ask the customer to confirm which account is theirs by verifying additional details (last 4 digits of payment method, address, etc.)","Refuse to help until the customer provides the exact email","Try the first variation and if nothing is found, tell the customer they don't have an account","Search for the most likely variation and proceed without confirming"],correct:0,explanation:"Searching multiple variations and verifying identity through additional factors is both helpful and secure. Proceeding without confirmation risks accessing the wrong account; refusing to help is unhelpful when simple verification can resolve the ambiguity."},
  { id:60,domain:"D1",scenario:"Customer Support Resolution Agent",question:"A travel booking agent helps a customer rebook a cancelled flight. During the rebooking process, the agent discovers the customer has an outstanding balance of $150 from a previous booking. Company policy says outstanding balances must be settled before new bookings. The agent should:",options:["Ignore the balance and complete the rebooking since the customer is already stressed","Inform the customer transparently about the outstanding balance, explain that it needs to be resolved before rebooking, help them settle the balance through available payment tools, and then proceed with the rebooking","Silently add the $150 to the new booking total","Cancel the rebooking process and tell them to call the billing department"],correct:1,explanation:"Transparency about the policy, helping resolve the blocker, and then completing the original request is the right flow. Hiding charges, ignoring policies, or deflecting to another department all create worse outcomes."},

  // ═══ DEEP SCENARIO: Code Generation with Claude Code (15 questions) ═══
  { id:61,domain:"D3",scenario:"Code Generation with Claude Code",question:"You're using Claude Code on a large monorepo with 500+ files. You need to add a new REST endpoint. Before writing any code, you should:",options:["Start writing the endpoint file immediately — Claude Code will figure out the patterns","Use Glob to find similar endpoint files, then Read 2-3 of them to understand the project's routing patterns, middleware usage, and response format before generating code","Ask Claude Code to scan every file in the repository first","Write the endpoint in isolation and refactor later to match the codebase"],correct:1,explanation:"Understanding existing patterns before generating code ensures consistency. Glob + Read on a few similar files gives Claude Code the right context without overwhelming the context window by scanning everything."},
  { id:62,domain:"D3",scenario:"Code Generation with Claude Code",question:"Your CLAUDE.md says: 'All API handlers must validate input using Zod schemas. Never use raw req.body.' Claude Code generates a handler that directly accesses req.body without Zod validation. This happened because:",options:["CLAUDE.md instructions are always followed perfectly — this can't happen","CLAUDE.md instructions are prompt-based guidelines, not programmatic enforcement. For critical patterns like input validation, you should also use hooks or linting in CI to catch violations","The CLAUDE.md file must be corrupted","Claude Code only reads CLAUDE.md once at startup and forgot"],correct:1,explanation:"CLAUDE.md provides guidelines via prompts — they're best-effort. If input validation is critical (and it usually is), enforce it through linting rules, pre-commit hooks, or CI checks in addition to CLAUDE.md."},
  { id:63,domain:"D3",scenario:"Code Generation with Claude Code",question:"You ask Claude Code to refactor a 2000-line file into smaller modules. Claude Code produces the refactored files, but several imports are broken. The best approach to fix this is:",options:["Manually fix each import yourself","Ask Claude Code to run the project's build/test command via Bash, read the error output, and iteratively fix each broken import","Start over and refactor more carefully this time","Revert the refactoring — it's too risky"],correct:1,explanation:"Using the Bash tool to run build/tests, then iteratively fixing errors based on actual compiler output is the correct Claude Code workflow. This is the validation-retry loop applied to code generation."},
  { id:64,domain:"D3",scenario:"Code Generation with Claude Code",question:"You have a project with directory-level CLAUDE.md files. The root CLAUDE.md says 'Use camelCase for variables.' The /src/database/ directory has its own CLAUDE.md that says 'Use snake_case for all database-related variables.' When Claude Code generates code in /src/database/models.js, it should:",options:["Use camelCase because root-level CLAUDE.md takes priority","Use snake_case because directory-level CLAUDE.md overrides root for that directory","Use a mix of both depending on context","Prompt the user to resolve the conflict"],correct:1,explanation:"Directory-level CLAUDE.md overrides project-level for files in that directory. This is the configuration hierarchy: more specific settings win."},
  { id:65,domain:"D3",scenario:"Code Generation with Claude Code",question:"You're using Claude Code in plan mode to evaluate a PR that changes authentication logic across 8 files. The plan identifies that the PR removes rate limiting from the login endpoint. You should:",options:["Approve the PR since Claude Code reviewed it","Flag the rate limiting removal as a critical security concern in the review output — removing rate limiting from login endpoints exposes the system to brute-force attacks","Ignore it — rate limiting is a performance feature, not a security feature","Only review the files with the most line changes"],correct:1,explanation:"Rate limiting on login endpoints is a security control against brute-force attacks. Plan mode's architectural analysis should catch this kind of cross-cutting security concern, which is exactly why multi-file review matters."},
  { id:66,domain:"D3",scenario:"Code Generation with Claude Code",question:"You need Claude Code to generate a database migration that adds a new column to a table with 50 million rows. Claude Code generates: ALTER TABLE users ADD COLUMN preferences JSONB DEFAULT '{}'. What's the issue and what should you instruct Claude Code to do?",options:["The migration is fine — ship it","The migration may lock the table for extended time on large tables. Instruct Claude Code to generate a non-locking migration strategy: use ALTER TABLE ... ADD COLUMN without a default (in PostgreSQL, adding a column without default doesn't rewrite the table), then backfill in batches","Add an index at the same time for efficiency","Use a different data type instead of JSONB"],correct:1,explanation:"On large tables, adding a column with a default can cause a full table rewrite and extended lock. Claude Code should generate production-safe migrations that consider table size — batch backfills and non-locking DDL."},
  { id:67,domain:"D3",scenario:"Code Generation with Claude Code",question:"You're writing tests with Claude Code. It generates 20 unit tests for a payment processing module, and all tests pass. However, every test mocks the payment gateway. What's the concern?",options:["No concern — mocking external services is best practice","The tests verify internal logic but never test actual gateway integration. You should ask Claude Code to also generate integration tests that hit a sandbox/test gateway, or at minimum contract tests that verify the mock matches the real API's interface","Mocking should be removed entirely — always use real services in tests","20 tests is too many — reduce to 5"],correct:1,explanation:"All-mock test suites can pass while the real integration is broken. Contract tests or integration tests against a sandbox ensure the mocks accurately represent the real gateway's behavior."},
  { id:68,domain:"D3",scenario:"Code Generation with Claude Code",question:"You ask Claude Code to 'add error handling to the payment flow.' It wraps every function in try/catch blocks that log the error and return null. This is:",options:["Good — all errors are caught","A problematic pattern. Catching every error and returning null hides failures, makes debugging difficult, and can cause silent data corruption downstream. Instead, errors should be caught at appropriate boundaries with specific handling per error type, and unrecoverable errors should propagate.","Fine for development, but needs improvement for production","The only way to prevent crashes"],correct:1,explanation:"Blanket try/catch with null returns is the 'silent error suppression' anti-pattern. Errors should be handled specifically at appropriate boundaries — some retried, some propagated, some reported — never silently swallowed."},
  { id:69,domain:"D3",scenario:"Code Generation with Claude Code",question:"Claude Code generates a React component and you want to verify it renders correctly. Rather than just reading the code, the most thorough verification in Claude Code is:",options:["Read the code carefully — if it looks right, it probably is","Run the project's test suite via Bash, and if no visual tests exist, generate a test that renders the component and asserts on key elements. For complex UI, generate a Storybook story or use a snapshot test.","Deploy it to production and check manually","Ask Claude Code to explain what the component does"],correct:1,explanation:"Actually running tests via Bash is always more reliable than code review alone. Generating tests for new components ensures they work and provides ongoing regression protection."},
  { id:70,domain:"D3",scenario:"Code Generation with Claude Code",question:"You ask Claude Code to implement a caching layer for API responses. It generates an in-memory cache with no expiration, no size limit, and no cache invalidation. What feedback should you give?",options:["It's a good start — optimize later","Reject and specify requirements: 'Add TTL-based expiration (configurable per endpoint), LRU eviction with a max size limit, and cache invalidation when the underlying data changes. Also consider what happens with multiple server instances — an in-memory cache won't be shared across them.'","Just add a TTL and it's fine","Caching is premature optimization — remove it"],correct:1,explanation:"Unbounded caches with no expiration cause memory leaks and serve stale data. Production caching needs TTL, eviction, invalidation, and consideration of multi-instance deployment. Being specific about requirements produces better code."},
  { id:71,domain:"D3",scenario:"Code Generation with Claude Code",question:"You're using Claude Code to debug a production issue: API response times jumped from 200ms to 3 seconds. You have access to the codebase but not production logs. The best debugging approach with Claude Code is:",options:["Ask Claude Code to optimize all database queries","Use Grep to search for recent changes to the API endpoint, Read the relevant files, then use Bash to run profiling or load tests locally. Cross-reference recent commits (git log) to identify what changed.","Rewrite the entire endpoint","Increase the server timeout to 5 seconds"],correct:1,explanation:"Systematic debugging: identify what changed (git log, Grep for recent edits), understand the current code (Read), and reproduce/profile locally (Bash). This follows the pattern of narrowing down the cause rather than guessing."},
  { id:72,domain:"D3",scenario:"Code Generation with Claude Code",question:"You need Claude Code to generate an API endpoint that handles file uploads. The generated code saves uploaded files to the local filesystem at the path provided by the user in the filename field. The security issue is:",options:["No issue — filenames from users are fine","Path traversal vulnerability. A malicious user could upload with filename '../../etc/passwd' and overwrite system files. Claude Code should generate code that sanitizes filenames (strip path separators, use a UUID-based name), validates file types, enforces size limits, and saves to a dedicated upload directory.","The issue is just file size — add a limit","File upload endpoints should always be handled by a CDN, not application code"],correct:1,explanation:"Unsanitized user-provided filenames are a classic path traversal attack vector. Claude Code should generate secure-by-default code: sanitized filenames, type validation, size limits, and sandboxed storage directories."},
  { id:73,domain:"D3",scenario:"Code Generation with Claude Code",question:"You ask Claude Code to add authentication to an Express.js API. It generates middleware that checks for a JWT in the Authorization header. The middleware decodes the JWT but does NOT verify the signature. This is:",options:["Fine — decoding is the same as verifying","A critical security vulnerability. Decoding a JWT only reads the payload; anyone can craft a JWT with any payload without knowing the secret. The middleware MUST verify the signature using the server's secret key (e.g., jwt.verify() not jwt.decode()). This is the difference between authentication and reading a plain-text header.","Acceptable for internal APIs","Minor — can be fixed later in a security review"],correct:1,explanation:"jwt.decode() vs jwt.verify() is a critical distinction. Without signature verification, any attacker can forge tokens. This must be caught and fixed immediately, not deferred."},
  { id:74,domain:"D3",scenario:"Code Generation with Claude Code",question:"You're using Claude Code to add a feature to a Django project. After generating the code, you realize Claude Code used function-based views, but the entire project uses class-based views (CBVs). You should:",options:["Accept it — function views work fine","Ask Claude Code to rewrite using class-based views. Also add a note to your CLAUDE.md: 'This project uses Django class-based views (CBVs). Never generate function-based views.' This prevents the pattern mismatch from recurring.","Keep both styles — variety is fine","File a bug report against Claude Code"],correct:1,explanation:"Consistency with existing codebase patterns matters for maintainability. Fix the immediate issue and update CLAUDE.md to prevent recurrence — this is exactly what CLAUDE.md is for: documenting project conventions."},
  { id:75,domain:"D3",scenario:"Code Generation with Claude Code",question:"Claude Code is generating a data processing pipeline. It creates a function that loads a 2GB CSV file entirely into memory using pandas.read_csv(). For a production system, you should instruct Claude Code to:",options:["It's fine — servers have enough memory","Generate a chunked processing approach: use pandas chunksize parameter or a streaming library to process the file in manageable batches, track progress, and handle errors per-chunk so a failure at row 1.5 million doesn't lose all previous processing.","Just increase the server's RAM","Use a different programming language"],correct:1,explanation:"Loading large files entirely into memory causes OOM crashes in production. Chunked processing with per-chunk error handling is the production-ready pattern — it's memory-efficient and resilient to partial failures."},

  // ═══ DEEP SCENARIO: Multi-Agent Research System (20 questions) ═══
  { id:76,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator dispatches 4 research subagents in parallel: literature_reviewer (searches academic papers), data_analyst (processes datasets), expert_interviewer (schedules expert calls), and trend_forecaster (analyzes market trends). The literature_reviewer fails with 'Authentication failed: API key expired'. The coordinator should:",options:["Abort immediately and ask the user to refresh credentials","Silently skip literature_reviewer and return results from the 3 successful agents","Retry literature_reviewer with exponential backoff (wait 2s, 4s, 8s), return structured error context if all retries fail: { failed_agent: 'literature_reviewer', reason: 'auth_expired', impact: 'academic sources unavailable', partial_results: {data_analyst, expert_interviewer, trend_forecaster} }","Automatically switch to a different academic database"],correct:2,explanation:"Retry with backoff + structured error context is production-ready. Silent suppression masks data gaps; aborting entirely wastes parallel work that succeeded; auto-switching without user awareness changes the scope of research."},
  { id:77,domain:"D1",scenario:"Multi-Agent Research System",question:"The coordinator must send context to subagents. Currently, the full 60,000-token research brief is stored in the coordinator's session. If a subagent only needs the 'market trends' section, the coordinator should:",options:["Pass the entire 60k-token brief to all subagents for consistency","Explicitly extract and pass ONLY the relevant 3k-token section to the subagent: 'You are analyzing market trends for healthcare AI startups. Focus on these metrics: [extracted section]. Ignore other research areas.'","Store context in a shared database and give subagents read-only access","Assume subagents will filter context automatically"],correct:1,explanation:"Explicit, focused context propagation reduces token waste and prevents information overload. Context is never automatically inherited or filtered by subagents."},
  { id:78,domain:"D1",scenario:"Multi-Agent Research System",question:"The coordinator must decide: dispatch all 4 subagents sequentially (A→B→C→D) or in parallel (A+B+C+D simultaneously). Which factors are most important?",options:["Sequential is always safer","Dependencies: if subagent B needs A's output, sequence them. Otherwise, parallel execution is faster and cheaper (fewer API calls, same cost, results ready sooner)","Parallel is always faster, so always choose it","Complexity — only use parallel for simple tasks"],correct:1,explanation:"Dependencies determine sequencing. If independent, parallel reduces latency and improves user experience at no extra cost."},
  { id:79,domain:"D1",scenario:"Multi-Agent Research System",question:"After all subagents complete, the coordinator has 4 result objects with conflicting findings: literature_reviewer says 'market declining', trend_forecaster says 'market growing', data_analyst says 'inconclusive', expert_interviewer says 'growing, but with caveats'. The coordinator should:",options:["Pick the most optimistic result (trend_forecaster)","Present all perspectives with source attribution: 'Literature suggests decline [source: academic papers from 2022-2024]. However, current market data shows growth [source: recent datasets]. Experts note growth with caveats [source: 5 expert interviews]. Conclusion: mixed signals — growth is recent, may be vulnerable to downturns.'","Average them into a single statement","Return only the expert opinion since humans are most reliable"],correct:1,explanation:"Conflict resolution means transparent aggregation with provenance. This allows users to weigh evidence and make informed decisions. Picking one source or averaging obscures the actual disagreement."},
  { id:80,domain:"D1",scenario:"Multi-Agent Research System",question:"A subagent is assigned 5 tools: search_academic_papers (API quota: 100/day), extract_keywords (local), rank_by_relevance (local), filter_by_date (local), generate_summary (LLM). The subagent receives 200 search requests in a day. What happens?",options:["All requests succeed because local tools are unlimited","First 100 searches succeed, then API hits quota. Requests 101-200 fail with 'quota_exceeded'. The coordinator should have: 1) Distributed requests across multiple days, 2) Added caching for duplicate searches, 3) Assigned quota-aware retry logic to the subagent.","The tool automatically batches requests","Requests wait in queue indefinitely"],correct:1,explanation:"Resource constraints are real. Subagent tool assignments must account for API quotas, and coordinators must distribute load intelligently or implement batching/caching."},
  { id:81,domain:"D1",scenario:"Multi-Agent Research System",question:"The coordinator uses a 'hub-and-spoke' architecture where subagents don't communicate with each other. What is NOT a valid reason for this design choice?",options:["Subagent tasks are independent (e.g., literature search ≠ data analysis)","Parallel execution is simpler when there's no inter-agent messaging","To reduce debugging complexity — all communication goes through coordinator","Subagents have different knowledge domains and tool specializations that don't naturally combine","Coordinator can monitor all interactions for oversight and quality control"],correct:1,explanation:"Hub-and-spoke is about independent, parallel tasks with central coordination. If tasks are independent (option 1, 2, 3, 4 are valid), hub-and-spoke works. If tasks are dependent, chain or mesh architectures are more natural."},
  { id:82,domain:"D1",scenario:"Multi-Agent Research System",question:"An agent generates a research summary, then another instance of the same agent ('review agent') is asked to critique it in the SAME SESSION. The review is positive. What's the risk?",options:["Review is efficient since it's in the same session","Same-session self-review retains the original reasoning bias. The review agent inherits the coordinator's context, which includes the original summary's framing. To avoid bias, use: 1) Separate session for review, 2) Different agent, or 3) External human reviewer. Same-session review is unreliable.","No risk — LLMs are objective","Review in same session reduces token usage, so it's always better"],correct:1,explanation:"Same-session self-review is a known anti-pattern. The agent retains reasoning bias from the original context. Always use separate sessions, different agents, or human review for critical validation."},
  { id:83,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator is building an information provenance system. For each fact in the final report, it must track: source (which subagent?), date retrieved, original source (which API/database?), confidence (based on data recency and source reliability). Which piece of information should NOT be tracked?",options:["Source subagent (e.g., 'data_analyst')","Original API/database (e.g., 'Crunchbase API v2.3')","The model's exact internal reasoning","Date retrieved and data freshness"],correct:2,explanation:"Provenance is about external traceability. The model's internal reasoning (token attention, logit patterns) is not useful for users understanding data sources. Track: subagent, API, timestamp, confidence."},
  { id:84,domain:"D1",scenario:"Multi-Agent Research System",question:"A research coordinator needs to handle long-running sessions (5+ hours of continuous querying). As context grows from 8k to 35k tokens, performance degradation appears: responses slow, quality drifts. The coordinator should:",options:["Increase context window to 200k to prevent any limits","Implement active context management: 1) Periodically summarize finished research into a brief, 2) Archive old queries to external storage, 3) Explicitly reference summaries in new queries. Example: 'Previous findings [see summary doc] show X. Now investigate Y. Do not duplicate prior research.'","Restart the session every 2 hours","Use a smaller model to save context"],correct:1,explanation:"Active context management is production-ready. Summarization + archival + explicit references maintain quality without wasting tokens on full context duplication."},
  { id:85,domain:"D1",scenario:"Multi-Agent Research System",question:"The coordinator assigns each subagent 4-5 specialized tools. The literature_reviewer has: search_academic_papers, filter_by_peer_review_status, extract_citations, rank_by_impact_factor, generate_literature_summary. Why is this assignment better than giving one subagent 15 tools?",options:["Fewer tools is faster","Focused tool assignment reduces confusion and improves accuracy. 15 tools per agent significantly degrades selection accuracy — the agent may call irrelevant tools or use wrong ones. 4-5 tools keeps the agent specialized.","More tools allows more parallelism","15 tools per agent is actually better, this is a trick question"],correct:1,explanation:"Tool overload degrades performance empirically. 4-5 focused tools per agent is optimal for accuracy and speed. This also prevents cross-domain confusion (e.g., literature_reviewer shouldn't have database tools)."},
  { id:86,domain:"D1",scenario:"Multi-Agent Research System",question:"A subagent fails: trend_forecaster returns { error: 'Insufficient data for forecast', prediction: null }. The coordinator already has 3 successful results. Should the coordinator:",options:["Treat it as a hard failure and escalate to user","Partial success: include the 3 successful results in the report, explicitly note that trend forecast is unavailable, explain why (insufficient data), and suggest user remediation: 'To improve forecasts, provide historical quarterly data for the past 3 years.'","Synthesize a forecast from the other 3 results as a substitute","Retry immediately without backoff"],correct:1,explanation:"Structured error handling with user-actionable context is correct. Partial success allows the report to be useful while being transparent about limitations."},
  { id:87,domain:"D1",scenario:"Multi-Agent Research System",question:"Two subagents are processing queries. The coordinator has 2 API call options: 1) Synchronous: call subagent A, wait for response, then call subagent B. 2) Asynchronous/batch: send both requests in a batch API call. Which is correct?",options:["Always synchronous for consistency","It depends on workflow: synchronous if B depends on A's output (blocking), batch/async if they're independent (latency-tolerant, cost-effective)","Always batch for speed","Use synchronous for simple, async for complex tasks"],correct:1,explanation:"Dependencies determine the choice. Blocking workflows need synchronous. Independent workflows benefit from batching (same cost, lower latency)."},
  { id:88,domain:"D1",scenario:"Multi-Agent Research System",question:"The coordinator has conflicting constraints: context is at 32k tokens (70% of 45k window), and it needs to dispatch 3 more subagent queries. What is the best strategy?",options:["Ignore the constraint and proceed","Before dispatching new queries, summarize completed research into a brief (e.g., 'Initial analysis shows X, Y findings detected from 200+ sources'). Replace the detailed conversation history with this summary. This frees ~10-15k tokens, allowing safe context management.","Reduce subagent query depth to save tokens","Cancel the 3 remaining queries"],correct:1,explanation:"Context summarization + history replacement is the standard production pattern. Preserve findings, discard verbose intermediate steps. Allows continuation without hitting limits."},
  { id:89,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator dispatches subagent A, which internally dispatches sub-subagent A1. Sub-subagent A1 fails. How is this failure communicated back to the coordinator?",options:["Automatically propagated up the chain","Subagent A must explicitly catch and communicate A1's failure to coordinator in a structured error response. Failure NEVER auto-propagates. Example: coordinator receives { status: 'partial_failure', succeeded: [A1_result], failed: [{agent: 'A1', reason: 'timeout', retry_attempts: 2}] }","Lost — coordinator has no way to know","A1 failure causes the entire coordinator to fail"],correct:1,explanation:"Explicit error communication is essential. Failures don't auto-propagate. Nested failures must be explicitly surfaced by parent agents in structured responses."},
  { id:90,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator is choosing between retrying a failed subagent call with: 1) Fixed delay (1s, 1s, 1s), 2) Linear backoff (1s, 2s, 3s), 3) Exponential backoff (1s, 2s, 4s). If the subagent is temporarily overloaded, which is best?",options:["Fixed delay — simplest","Exponential backoff is best for transient failures (brief overloads). It gives the system time to recover while still retrying aggressively. Linear backoff is a middle ground. Fixed delay can overwhelm a busy system.","Linear backoff — balanced","No retries needed, just fail immediately"],correct:1,explanation:"Exponential backoff is standard for transient failures. Fixed delays can compound overload; exponential gives breathing room."},
  { id:91,domain:"D1",scenario:"Multi-Agent Research System",question:"A subagent is assigned a 'check_data_quality' tool that validates datasets. Another subagent is assigned 'download_raw_data' tool. The coordinator receives contradictory reports: subagent 1 says data is 'high quality', subagent 2 says 'download failed'. The coordinator should:",options:["Trust the quality assessment since it's explicit about high quality","These report different concerns (quality vs availability). Correct interpretation: 'Data is available and high quality [from subagent 1], but downstream system cannot currently download it [from subagent 2]. Root cause: likely access permission or connectivity issue with download service, not data quality itself.'","Believe whichever subagent ran first","They're both lying, restart from scratch"],correct:1,explanation:"Tool specialization means different subagents surface different concerns. Quality and availability are separate dimensions. Correct interpretation requires understanding what each tool measures."},
  { id:92,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator manages a multi-agent research system with internal checkpointing. After subagent A completes, coordinator saves state: { subagent_A_results: {...}, timestamp: T1 }. If the session crashes at T2, and is resumed, what should the coordinator do?",options:["Ignore the checkpoint and re-run everything from scratch","Resume from checkpoint: skip re-running subagent A, restore A's results, continue from T1 onward. This saves compute and API costs while maintaining consistency.","Merge old and new results in undefined ways","Never use checkpoints in multi-agent systems"],correct:1,explanation:"Checkpointing + resumption is critical for long-running systems. Restoring from checkpoint prevents wasted API calls and maintains continuity."},
  { id:93,domain:"D1",scenario:"Multi-Agent Research System",question:"Two subagents report contradictory confidence in their findings: literature_reviewer says 'confidence: 85% (based on 50 papers analyzed)', expert_interviewer says 'confidence: 40% (based on 3 interviews, experts disagreed)'. How should the coordinator report this?",options:["Report only the highest confidence to be optimistic","'Research is uncertain. Literature review (50 sources) suggests high confidence (85%), but expert interviews (3 participants) show disagreement and low confidence (40%). Conclusion: findings are supported by literature but lack expert consensus. Recommend obtaining more expert input for validation.'","Average them: 62.5% confidence","Report them separately without synthesis"],correct:1,explanation:"Transparent aggregation with source context. Users need to understand the confidence gradient and why experts disagree. This guides decision-making better than single scores or averaging."},
  { id:94,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator discovers that subagent A's tool (search_competitor_pricing) has a rate limit of 10 calls/minute. Subagent A is making 25 calls/minute. The coordinator should:",options:["Ignore the limit and let the calls fail","Implement throttling: queue subagent A's requests at ≤10/min. Use a leaky bucket or token bucket algorithm. This prevents quota exhaustion and allows controlled throughput.","Pre-fetch all data in bulk","Use a different tool with no limits"],correct:1,explanation:"Rate limit awareness is critical. Throttling prevents quota exhaustion and maintains system stability. Token bucket algorithms are standard for this."},
  { id:95,domain:"D1",scenario:"Multi-Agent Research System",question:"A long-running coordinator session has processed 50 research queries, spanning 20,000 tokens of conversation history. New queries are now slower (latency increased from 2s to 8s). The context window is at 60% (27k/45k). What is the root cause?",options:["Model is degrading over time","Lost-in-the-middle effect: middle portions of the 20k-token history receive less attention than recent tokens, causing degraded reasoning. Solution: summarize old queries into brief findings, archive verbose conversation, keep only recent context and brief history.","Server is overloaded","No root cause — slowness is normal for long sessions"],correct:1,explanation:"Lost-in-the-middle is well-documented. At ~60% capacity with long history, middle content (e.g., queries 15-35) loses precision. Summarization + archival solve this."},

  { id:96,domain:"D1",scenario:"Multi-Agent Research System",question:"A synthesis agent receives results from 5 subagents. Subagent #3 returned { error: 'API connection timeout', attempts: 3, lastRetryTime: 1234567890 }. The synthesis agent logs: 'Subagent 3 failed.' Should the coordinator:",options:["Insert a preprocessing agent to validate all subagent outputs before synthesis","Update subagent #3 to return structured error info: { errorType: 'transient_timeout', retriedAt: [times], nextRetryDelay: 'exponential' }. Then synthesis receives rich context to adapt gracefully.","Add a gatekeeper layer to filter out failures before synthesis","Ask subagents to re-run on any error without context"],correct:1,explanation:"Passing rich context forward (error type, retry history, timing) lets synthesis make intelligent decisions. Gatekeepers and preprocessing layers are anti-patterns that hide important signal."},

  { id:97,domain:"D1",scenario:"Multi-Agent Research System",question:"The coordinator receives summary outputs from subagents. Market_analyzer returns just: { summary: 'Market is growing 5% YoY' }. Sentiment_analyzer returns: { summary: 'Sentiment is positive' }. The coordinator's user-facing report is vague. What should change?",options:["Add a summary-aggregation layer to distill findings further","Require subagents to return structured output: { insight: '...', sources: N, confidence: X%, limitations: '...' }. This gives the coordinator richer material for synthesis without adding preprocessing layers.","Insert an intermediate refinement agent","Concatenate summaries as-is"],correct:1,explanation:"Structured upstream output (sources, confidence, limitations) flows naturally to synthesis. No preprocessing layer needed. The problem is poor upstream format, not missing middleware."},

  { id:98,domain:"D1",scenario:"Multi-Agent Research System",question:"Subagent A reports findings with high confidence but provides zero evidence. Coordinator receives: { finding: 'Tech adoption will spike 40% next Q', confidence: 0.95, details: null }. Coordinator should:",options:["Add a verification layer to second-guess the subagent","Require subagent A to return structured context: { finding: '...', confidence: X, reasoning: '...', sources: [...], limitations: '...' }. Coordinator uses this richer output to evaluate claims.","Trust the confidence score without evidence","Reduce confidence artificially before passing to synthesis"],correct:1,explanation:"Pushing for structured output (reasoning, sources, limitations) upstream avoids the need for downstream verification layers. Context flows forward, not gatekeepers inserted backward."},

  { id:99,domain:"D1",scenario:"Multi-Agent Research System",question:"A subagent fails mid-query with status: 'error'. The coordinator doesn't know if this was a timeout, data not found, permission denied, or corrupted response. What is the best fix?",options:["Add an error-classification agent after each subagent","Subagent must return: { status: 'error', errorType: 'timeout|not_found|permission|corruption|other', query: '...', partialResults: [...], suggestedAlternative: '...' }. Coordinator receives categorized context.","Treat all errors identically","Preprocess error messages into a generic format"],correct:1,explanation:"Structured error context (type, query, partials, alternatives) lets coordinator make fine-grained decisions. Adding a classifier is middleware bloat; fix the source format."},

  { id:100,domain:"D1",scenario:"Multi-Agent Research System",question:"Coordinator dispatches 3 queries to a research subagent. Query 1 and 3 succeed; Query 2 times out after 5s. The coordinator currently logs: 'Subagent processed 2/3 queries.' Users don't know which failed. Solution:",options:["Add a logging layer to track query IDs","Subagent returns: { successCount: 2, failureCount: 1, results: [...], failures: [{ query: 'X', reason: 'timeout', elapsed: 5s, retry: 'failed_after_2_attempts' }] }. Coordinator now has granular context.","Fail the entire request","Retry all 3 queries"],correct:1,explanation:"Rich structured output (per-query status, timing, retry context) eliminates the need for logging layers. Context passes forward natively."},

  { id:101,domain:"D1",scenario:"Multi-Agent Research System",question:"A subagent encounters a timeout. Another encounters 0 results. Another encounters corrupted JSON. The coordinator currently treats all three as 'error' and logs them identically. The risk is:",options:["Coordinator is using good error handling","Different error types require different recovery: timeout should retry exponentially, 0-results should try alternatives, corruption should escalate to user. Treating them identically hides recovery options.","All errors are the same","Errors should be cached, not recovered"],correct:1,explanation:"Failure type distinction is critical. Conflating timeout + not_found + corruption loses the ability to respond appropriately. Structure errors by type."},

  { id:102,domain:"D1",scenario:"Multi-Agent Research System",question:"Subagent A fails with a transient timeout. Subagent B fails because the requested resource doesn't exist. The coordinator currently retries both with the same exponential backoff strategy. What's the problem?",options:["Nothing, retrying both is correct","Timeout: retry (system will recover). Not-found: don't retry (won't change). Retrying not-found wastes API quota and time. Coordinator must distinguish and apply per-type recovery strategy.","All failures should be retried once","Failures should never be retried"],correct:1,explanation:"Failure-type awareness drives recovery strategy. Transient failures benefit from retry; terminal failures (not-found) don't. Blanket retry is inefficient and blind."},

  { id:103,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator aggregates confidence scores from 3 subagents into a single metric: (0.8 + 0.3 + 0.95) / 3 = 0.68. The report says 'Overall confidence: 68%'. Users are confused about which areas are weak. Solution:",options:["Remove all confidence scores","Report with granularity: 'Data quality high (95%), market analysis moderate (80%), trend forecast low (30% — insufficient historical data). Recommend strengthening trend forecast with quarterly data before 2020.'","Average to a higher precision (68.33%)","Use only the highest confidence (95%)"],correct:1,explanation:"Aggregating into a single metric hides critical distinctions. Users need to see confidence *per area* and understand *why* some are weak, guiding follow-up actions."},

  { id:104,domain:"D1",scenario:"Multi-Agent Research System",question:"A research system retrieves data from 100 sources. Only 40 are successfully analyzed; 60 encounter errors (network failures, parsing errors, access denied). Coordinator reports: 'Analysis complete: confidence 0.40.' What's missing?",options:["Nothing, 40% confidence is accurate","Distinguish error types: 'Analysis covered 40/100 sources (40% coverage). Failures: 35 network (retryable), 20 access-denied (need credentials), 5 parse errors (malformed data). Recommend: retry network calls, request missing creds, escalate parse errors.'","Add more sources to increase coverage","Retry everything uniformly"],correct:1,explanation:"Transparent breakdown of coverage gaps and failure types drives action. Single confidence score hides what went wrong and why. Users can then prioritize remediation."},

  { id:105,domain:"D1",scenario:"Multi-Agent Research System",question:"The coordinator needs market research, competitive analysis, and risk assessment. Currently it dispatches all 3 subagents in parallel with the same prompt template. Two succeed; risk assessment returns null due to 'insufficient context'. The coordinator should:",options:["Blame the risk assessment subagent for poor implementation","This is a coordinator decomposition failure. Risk assessment needs more context (e.g., market size, competitive threats identified by earlier subagents). Coordinator must route outputs from market + competitive subagents *into* risk assessment for context chaining, not parallel dispatch.","Retry risk assessment with the same template","Remove risk assessment from the pipeline"],correct:1,explanation:"Coordinator is the brain: it must decompose tasks with awareness of dependencies. Parallel dispatch without context chaining sets subagents up for failure. Routing and synthesis are coordinator responsibilities."},

  { id:106,domain:"D1",scenario:"Multi-Agent Research System",question:"Three subagents complete: data_ingestion, data_analysis, report_generation. Data_ingestion found only 20 sources (expected 200). Report_generation synthesized findings anyway. The report is thin. Coordinator's response:",options:["Blame data_ingestion for insufficient coverage","Coordinator failed to detect the coverage gap (20 vs 200) and route appropriately. It should: 1) Recognize coverage shortfall at data_ingestion, 2) Route expanded query to data_ingestion, 3) Re-dispatch analysis, 4) Then report_generation. Recovery decisions live at coordinator.","Blame report_generation for poor synthesis","Accept the thin report"],correct:1,explanation:"Coordinator visibility and routing control coverage. Missing sources is a decomposition and sequencing problem, not a subagent problem. Coordinator must intervene."},

  { id:107,domain:"D1",scenario:"Multi-Agent Research System",question:"A coordinator's task: 'Analyze the market, include competitor data, assess risks.' It dispatches: 1 market_analyzer, 1 competitor_analyzer, 1 risk_assessor in parallel. Risk assessor fails: 'Need competitor data first.' What went wrong?",options:["Risk assessor is poorly designed","Coordinator didn't decompose task breadth: market + competitor have NO dependency order (parallel OK), but risk assessment *depends on* both. Correct flow: 1) Parallel: market + competitor, 2) Sequential: risk (gets both outputs), 3) Synthesis. Coordinator must model dependencies.","Risk assessor should predict its own inputs","All tasks are independent"],correct:1,explanation:"Coordinator owns task decomposition. It must identify blocking dependencies and sequence tasks accordingly. Dispatching risk after its inputs are ready is the coordinator's job, not the subagent's."},

  { id:108,domain:"D1",scenario:"Multi-Agent Research System",question:"Subagent A synthesizes findings from B, C, D. B succeeds, C times out, D returns partial results. Subagent A reports: 'Synthesis failed due to missing C.' The coordinator should:",options:["Retry subagent A","Coordinator should NOT expect A to synthesize with incomplete inputs. Instead: 1) Coordinator detects failure+partial state, 2) Routes a recovery decision: retry C, or provide A with B+D only + note the gap, 3) Re-trigger A with explicit context.","Fire subagent A","Cancel the entire request"],correct:1,explanation:"Coordinator is the brain: it must handle recovery. Subagent A shouldn't fail silently due to missing inputs; coordinator must decide: retry, degrade, or escalate. Recovery is a coordinator-level concern."},

  { id:109,domain:"D1",scenario:"Multi-Agent Research System",question:"Coordinator runs a long chain: A → B → C → D. Task D depends on C's output. C produces { results: [...], warnings: ['incomplete data'] }. Coordinator passes C's output directly to D without interpreting warnings. D fails: 'Data too sparse.' The root cause:",options:["Subagent D is flawed","Coordinator failed to synthesize or filter C's output for D. Warnings should trigger coordinator action: inspect data sparsity, decide: retry C with expanded scope, or alert user, or feed D with lower confidence. Coordinator can't just pass warnings through unchanged.","Warnings should be ignored","Add a filtering layer between C and D"],correct:1,explanation:"Coordinator is the synthesizer of outputs. It must interpret warnings, make decisions (retry, degrade, escalate), and modify context before routing to downstream tasks. Blind pass-through is not coordination."},

  { id:110,domain:"D1",scenario:"Multi-Agent Research System",question:"Coordinator manages 2 subagents: forecaster (12s latency) and validator (3s latency). Currently dispatcher sends both in parallel, waits 15s for both. Forecaster often times out at 10s. Coordinator should:",options:["Add a load-balancer agent","Change coordinator logic: dispatch forecaster on a longer timeout (e.g., 15s), dispatch validator immediately (3s). Route forecaster output to validator for cross-check. This is a coordinator sequencing decision, not a subagent fix.","Reduce both timeouts equally","Remove forecaster from pipeline"],correct:1,explanation:"Coordinator owns execution strategy. Different subagents have different latency profiles; coordinator must adapt timeouts and orchestration, not blame subagents or add middleware layers."},
];
